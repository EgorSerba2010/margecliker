...
let currentOfflineEarningsBuffer = 0; // Сюда прячем оффлайн-доход до клика по кнопкам
    ...
    if (savedData) {
        try {
            ...
            if (gameState.lastSaveTime) {
                ...
                if (secondsPassed > 10) {
                    let totalValue = 0;
                    // Рассчитываем доход со всех полей (с учетом твоих последних сетевых правок)
                    if (gameState.cardsByFields) {
                        Object.values(gameState.cardsByFields).forEach(fieldList => {
                            fieldList.forEach(c => totalValue += c.value);
                        });
                    ...
                    if (currentPPS > 0) {
                        ...
                        if (offlineEarnings > 0) {
                            ...
                            // Больше не начисляем деньги сразу на баланс. Прячем их в буфер!
                            currentOfflineEarningsBuffer = offlineEarnings;

                            const offlinePopup = document.getElementById('offline-popup');
                            const timeText = document.getElementById('offline-time-text');
                            
                            // Находим спан внутри тега h3, который мы переписали в HTML
                            const rewardValueSpan = document.getElementById('offline-reward-value');

                            if (offlinePopup && timeText && rewardValueSpan) {
                                ...
                                // Выводим базовую сумму на экран
                                rewardValueSpan.textContent = formatNumber(offlineEarnings);
...
// Функция А: Обычное получение награды (х1)
function claimNormalOfflineReward() {
    if (currentOfflineEarningsBuffer > 0) {
        updateBalance(currentOfflineEarningsBuffer);
        spawnFloatingText(`+${formatNumber(currentOfflineEarningsBuffer)} $`, "50%", "50%");
        
        // Очищаем буфер и закрываем окно
...
function claimDoubleOfflineReward() {
    ...
    AdController.show().then((result) => {
        ...
        lastAdWatchTime = Date.now(); // Фиксируем время просмотра для кулдауна ТВ-кнопки
        ...
        console.log("Удвоение отменено: реклама не досмотрена до конца.", result);
    });
}
